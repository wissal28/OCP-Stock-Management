...
export function computeEntreesForDay(operationalDay) {
  const rows = db.prepare("SELECT * FROM dechargements").all();
...
export function computeSortiesForDay(operationalDay) {
  const rows = db.prepare("SELECT repriseOuDirection, tonnage, dateDebutAffect, heureDebutAffect FROM chargements_portique").all();
...
export async function computeSnapshotForDay(operationalDay) {
  ...
  const previous = db
    .prepare("SELECT * FROM stock_snapshots WHERE date < ? ORDER BY date DESC LIMIT 1")
    .get(operationalDay);
  ...
  const { quantites: entrees, qualites: qualitesEntrees } = computeEntreesForDay(operationalDay);
  const sorties = computeSortiesForDay(operationalDay);
...
export function getOperationalDaysWithActivity() {
  const days = new Set();

  for (const d of db.prepare("SELECT date, debutDechargement FROM dechargements").all()) {
    const day = getOperationalDay(d.date, d.debutDechargement || "12:00");
    if (day) days.add(day);
  }
  for (const c of db.prepare("SELECT dateDebutAffect, heureDebutAffect FROM chargements_portique").all()) {
...
export async function backfillStockSnapshots(actorMatricule) {
  const days = getOperationalDaysWithActivity();
  const created = [];
...
export async function recomputeAllStockSnapshots(actorMatricule) {
  ...
  for (const day of days) {
    const existingRow = db.prepare("SELECT * FROM stock_snapshots WHERE date = ?").get(day);
    const existing = existingRow ? { ...existingRow, zones: JSON.parse(existingRow.zones) } : null;

    const previousRow = db.prepare("SELECT * FROM stock_snapshots WHERE date < ? ORDER BY date DESC LIMIT 1").get(day);
    ...
    const { quantites: entrees, qualites: qualitesEntrees } = computeEntreesForDay(day);
    const sorties = computeSortiesForDay(day);
...
function computeAllMovementsForDay(operationalDay) {
  const dechargements = db.prepare("SELECT * FROM dechargements").all();
  ...
  const chargements = db
    .prepare("SELECT repriseOuDirection, tonnage, dateDebutAffect, heureDebutAffect FROM chargements_portique")
    .all();
...
function computeCumulativeExistant(uptoDay, destinationIds) {
  const totals = new Map(destinationIds.map((id) => [id, 0]));
  const days = getOperationalDaysWithActivity().filter((day) => day <= uptoDay);
  for (const day of days) {
    const { entrantMap, sortantMap } = computeAllMovementsForDay(day);
...
export async function getCelluleSiloMovements(operationalDay) {
  const { entrantMap, sortantMap, navireDirectQuantite } = computeAllMovementsForDay(operationalDay);
  ...
  const cumulLot3 = computeCumulativeExistant(operationalDay, lot3Ids);
  const cumulSilos = computeCumulativeExistant(operationalDay, SILO_IDS);
  ...
  const peseuses = computePeseusePortiqueActivity(operationalDay);
 ...
 */
function computePeseusePortiqueActivity(operationalDay) {
  const isToday = operationalDay === getCurrentOperationalDayServer();
  const rows = db
    .prepare("SELECT portique, peseuse, tonnage, dateDebutAffect, heureDebutAffect, heureFinAffect FROM chargements_portique")
    .all();
