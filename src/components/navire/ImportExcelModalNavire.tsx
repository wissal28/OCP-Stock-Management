...
import type { ChargementPortique } from "./navireData";
import {
  IMPORT_TARGET_FIELDS,
  buildChargementsFromImport,
  parseExcelFile,
  type ImportPreview,
  type ImportTargetField
} from "./excelImportServiceNavire";
...
const FIELD_LABELS: Record<ImportTargetField, string> = {
  ...
  cale: "Cale",
  peseuse: "Peseuse",
  repriseOuDirection: "Rep ou dir",
  debutAffect: "Déb. affectation",
  finAffect: "Fin affectation",
  tonnage: "Tonnage"
};

export default function ImportExcelModalNavire({
  open,
  navireId,
  onClose,
  onImport
  ...
  navireId: string;
  onClose: () => void;
  onImport: (chargements: ChargementPortique[]) => void;
}) {
  ...
  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setImportErrors([{ rowNumber: 0, message: "Impossible de lire ce fichier. Le format .xls historique (avant Excel 2007) n'est pas supporté — réenregistrez-le au format .xlsx." }]);
    } finally {
      setLoading(false);
    }
  }
  ...
  function handleConfirmImport() {
    if (!preview) return;
    const { chargements, errors } = buildChargementsFromImport(preview, mapping, navireId);
    setImportErrors(errors);
    setImportedCount(chargements.length);
    if (chargements.length > 0) onImport(chargements);
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-[#102b20]/30 p-4 backdrop-blur-[2px]" onClick={handleClose}>
      <div onClick={(e) => e.stopPropagation()} className="my-6 w-full max-w-4xl rounded-lg border border-[#dfe6d7] bg-white p-5 shadow-[0_24px_60px_rgba(16,43,32,0.2)]">
        ...
        {!preview && (
          <label className="mt-5 flex cursor-pointer flex-col items-center justify-center gap-2 rounded-lg border-2 border-dashed border-[#b8c6bc] bg-[#f6f9f2] p-10 text-center">
            ...
            <span className="text-sm font-bold text-[#102b20]">{loading ? "Lecture du fichier…" : "Cliquez pour choisir un fichier .xlsx"}</span>
            <span className="text-xs font-semibold text-[#6c7c71]">La feuille "Pesée bascules" (navire/portique/cale/pese/rep ou dir/tonnage) sera détectée automatiquement.</span>
