...
export default function Aurora({
  ...
  useEffect(() => {
    ...
    const toRgb = (stops: [string, string, string]) =>
      stops.map((hex) => {
    ...
    const update = (t: number) => {
      ...
      program.uniforms.uTime.value = (current.time ?? t * 0.01) * (current.speed ?? 1) * 0.1;
      program.uniforms.uAmplitude.value = current.amplitude ?? 1;
      program.uniforms.uBlend.value = current.blend ?? 0.5;
      program.uniforms.uColorStops.value = toRgb(current.colorStops ?? colorStops);
