...
import { NATURE_ARRET_OPTIONS, type ArretTrain, type Train } from "./trainData";
import { calculateDuration, formatDuration } from "./trainRules";

const AXE_OPTIONS = ["DA10", "DB10"];
...
export default function ArretsTrainView({
  trains,
  arrets,
  onCreate,
  onUpdate,
  ...
  trains: Train[];
  arrets: ArretTrain[];
  onCreate: (arret: ArretTrain) => void;
  onUpdate: (id: string, patch: Partial<ArretTrain>) => void;
  onDelete: (id: string) => void;
}) {
  const [open, setOpen] = useState(false);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [form, setForm] = useState({ ...EMPTY_FORM, trainId: trains[0]?.id ?? "" });
  ...
  function startEdit(arret: ArretTrain) {
    setForm({
      trainId: arret.trainId,
      axe: arret.axe,
  ...
  function handleSubmit(event: React.FormEvent) {
    ...
    if (!form.trainId || !form.dateDebut || !form.heureDebut || !form.nature) return;

    if (editingId) {
      onUpdate(editingId, { ...form });
    } else {
      onCreate({
        id: `arret-train-${Date.now()}`,
    ...
    setForm({ ...EMPTY_FORM, trainId: form.trainId });
    closeForm();
  }

  const trainLabel = (id: string) => {
    const train = trains.find((t) => t.id === id);
    return train ? `Train ${train.numeroTrain}` : "—";
  };

  if (trains.length === 0) {
    return (
      <div className="rounded-lg border border-dashed border-[#b8c6bc] bg-white/70 p-10 text-center text-sm font-bold text-[#6c7c71]">
        Créez d'abord un train dans l'onglet « Trains & déchargement » pour enregistrer ses arrêts.
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="font-display text-xl font-medium text-[#102b20]">Arrêts de déchargement</h2>
          <p className="mt-1 text-sm text-[#6c7c71]">Journal des arrêts par train — même structure que le module navires.</p>
        </div>
        {!open && (
          <button
            type="button"
        ...
        <form onSubmit={handleSubmit} className="rounded-lg border border-[#dfe6d7] bg-white/95 p-5 shadow-[0_16px_42px_rgba(16,43,32,0.06)]">
          ...
          <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
            <label className="flex flex-col gap-1">
              <span className={labelClass}>Train</span>
              <select value={form.trainId} onChange={(e) => update("trainId", e.target.value)} className={inputClass}>
                {trains.map((t) => (
            ...
            <label className="flex flex-col gap-1">
              <span className={labelClass}>Flux (DA10/DB10)</span>
              <select value={form.axe} onChange={(e) => update("axe", e.target.value)} className={inputClass}>
            ...
            <label className="flex flex-col gap-1">
              <span className={labelClass}>Élément concerné</span>
            ...
            <tbody>
              {arrets.map((arret) => (
                <tr key={arret.id} className="border-b border-[#eef1ea] last:border-0 hover:bg-[#f6f9f2]">
                  <td className="px-4 py-3 font-black text-[#102b20]">{trainLabel(arret.trainId)}</td>
                  <td className="px-4 py-3 font-semibold text-[#314238]">{arret.axe || "—"}</td>
                  ...
                  <td className="px-4 py-3 font-semibold text-[#314238]">
                    {formatDuration(arret.heureFin ? calculateDuration(arret.heureDebut, arret.heureFin) : null)}
                  </td>
