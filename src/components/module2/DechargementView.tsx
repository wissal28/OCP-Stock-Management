...
const DECHARGEMENT_CIRCUITS = ["dechargement-da", "dechargement-db", "poste-66"];

export default function DechargementView({ onViewGlobal }: { onViewGlobal: () => void }) {
  const equipments = EQUIPMENTS.filter((equipment) => DECHARGEMENT_CIRCUITS.includes(equipment.circuitId));
    ...
    <div className="grid gap-5">
      <section className="relative overflow-hidden rounded-lg border border-[#dfe6d7] bg-white/85 p-5 shadow-[0_16px_42px_rgba(16,43,32,0.06)] backdrop-blur-md sm:p-6">
        ...
        <div className="relative flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.22em] text-[#0d6b4d]">Moyens de déchargement</p>
            <h2 className="font-display mt-2 text-2xl font-medium tracking-tight text-[#102b20] sm:text-3xl">Moyens de déchargement</h2>
            <p className="mt-2 max-w-2xl text-sm leading-6 text-[#6c7c71]">
              Réception du minerai déchargé côté wagons (circuits DA et DB) et transfert vers les trémies T10 du poste 66.
            </p>
          </div>
          <button
            type="button"
      ...
      <CircuitCards circuitIds={DECHARGEMENT_CIRCUITS} onVisualize={onViewGlobal} title="Familles de déchargement" />
