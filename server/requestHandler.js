// Routeur API pur : aucun effet de bord au chargement du module (pas de migration, pas de tâche
// planifiée, pas d'écoute réseau) — uniquement des imports et l'export de `handleRequest`. Réutilisé
// tel quel par deux points d'entrée : server/index.js (processus Node long-running, dev local et
// hébergeur avec serveur persistant) et netlify/functions/api.js (fonction serverless).
...
// En local (dev), l'origine autorisée reste le frontend Vite par défaut. En production, ALLOWED_ORIGIN
// doit pointer vers l'URL réelle du site déployé (variable d'environnement Netlify).
...
export async function handleRequest(request, response) {
  ...
  try {
    ...
    if (request.method === "POST" && url.pathname === "/api/auth/login") {
      ...
      const { token, expiresAt } = await createSession(user.matricule);
